...
![리워크 이후의 지면 — 강이 사구 사이를 굽이쳐 흐르고, 강변 자갈 레이어가 따로 칠해져 있다](/images/blog/factory-sim/s13-planet-vista.webp)
...
## 3. 바닥 머티리얼 — 그래프는 코드로, 붓질은 손으로

멀티레이어 랜드스케이프 머티리얼은 보통 머티리얼 에디터에서 노드를 손으로 잇는다. 이번엔 에디터 유틸리티 `UOJJ_LandscapeMaterialFactory`를 만들어 **머티리얼 그래프 자체를 코드로 생성**했다. 레이어는 2종 — 일반 지대의 기본 지면과 강 주변의 자갈 — 을 `LandscapeLayerBlend`로 섞고, 파라미터(NormalIntensity 2.0, MacroVariationAmount 0.4 등)를 인스턴스로 노출하는 구조다.

그래프가 코드에서 나온다고 해서 페인팅까지 자동은 아니다. **강 주변 자갈 레이어는 에디터에서 직접 붓으로 칠했다.** 강줄기를 따라 어디까지가 자갈밭인지는 수식보다 눈이 정확하다. 생성기는 재료(머티리얼 그래프)를 일관되게 찍어내는 공장이고, 그 재료를 어디에 바를지는 레벨 위의 수작업 — 코드 생성과 손 페인팅이 한 머티리얼 안에서 분업하는 조합이 이번 리워크에서 꽤 마음에 든 부분이다.
...
단, **노멀맵만은 이 트릭에서 제외**했다. 생성기 소스에 경고 주석으로 박아 둔 이유 그대로 —
...
색은 평균해도 색이지만, 노멀은 방향 벡터라 서로 다른 스케일을 평균하면 섭동이 상쇄돼 입체감이 사라진다. 텍스처라고 다 같은 텍스처가 아니다.
...
- 멀티레이어 바닥 머티리얼(기본 지면 + 강변 자갈 2레이어)을 에디터 유틸리티 코드로 생성, 강변 자갈은 손 페인팅 — 멀티스케일 UV로 타일 반복 깨기, 노멀만 단일 스케일
