...
## 5. `UpdateHoverPreview` — 풋프린트 단위 판정

> 📝 **정정 노트 (2026-06-05):** 이 절은 처음 발행 때 호버 색을 *셀마다 따로* 칠하는 구현으로 설명했습니다. 초기 구현은 셀별 분기였고, 이후 호버 색을 풋프린트 전체에 대해 `CanPlaceMachine` **한 번**으로 정하는 방식(클릭 판정과 같은 함수 → 단일 진실원)으로 단순화했습니다. 아래 코드·설명을 현재 코드 기준으로 보정했어요 — 경위는 [5편](/blog/factory-sim/05-codex-adversarial-review)에서 다룹니다.
...
> ℹ️ `RotationSteps` 인자는 이후 회전 기능에서 추가된 것입니다([4편](/blog/factory-sim/04-mouse-input-build-controller) 참고). 이 글 범위에선 0이라고 보면 되고, 핵심은 *색을 셀마다가 아니라 풋프린트 전체에 한 번* 정한다는 점이에요.

읽기 순서대로 짚어볼게요.

1. **`ClearHoverPreview()`** — 매 호출의 첫 줄. 이전 프레임 인스턴스를 싹 지운다. 누적되면 마우스를 흔드는 즉시 화면이 녹/빨 카펫이 됩니다.
2. **`CanPlaceMachine` 한 번으로 색 결정** — 풋프린트에 한 칸이라도 점유·out-of-bounds가 있으면 `bCanPlace`가 `false`라 전체가 `InvalidHoverISM`(빨강)로 간다. 클릭 때 호출하는 그 `CanPlaceMachine`과 **같은 함수**라, 보이는 색과 실제 배치 결과가 어긋날 수 없다.
3. **풋프린트 셀 순회 — 위치만 계산** — 색은 이미 정해졌고, 루프는 각 셀에 인스턴스를 *어디에* 놓을지(트랜스폼)만 만든다. `CalculateFootprint`는 머신 크기 기준 차지 셀 목록(2편 참고).
4. **Z + 2 오프셋** — `GridToWorld`가 돌려주는 셀 중심 Z는 그리드 액터 Z(보통 0)와 동일. 거기에 +2를 더해 호버 인스턴스를 world Z=2로 띄웁니다. 베이스 그리드 평면이 Z=1에 깔려 있어 그 위 1 유닛 차이로 가림 방지. 빠뜨리면 Z-fighting으로 깜빡거립니다.
5. **CellSize에 맞춘 스케일** — UE 엔진 기본 Plane 메시는 100×100 단위. 셀 사이즈가 200이면 2배, 50이면 0.5배.
6. **`bWorldSpace=true`** — 인스턴스 좌표를 월드 기준으로 해석. 그리드 액터를 어디 두든 셀 중심이 정확하게 잡힙니다.

### 왜 셀별이 아니라 풋프린트 전체인가

2×2 머신을 그리드 모서리에 가져가면, anchor(왼쪽 아래)는 유효 셀인데 나머지 셀들은 그리드 바깥일 수 있어요. 색을 *셀마다* 정하면 그 바깥 칸만 빨강, 안쪽은 녹색인 — 섞인 미리보기가 나옵니다. 그런데 `CanPlaceMachine`은 한 칸이라도 막히면 통째로 거부하니, 사용자는 "녹색도 있으니 놓이겠지" 하고 클릭했다가 거부당해요. **시각 피드백이 거짓말**을 한 겁니다. (이게 초기 셀별 분기 구현의 함정이었어요.)

그래서 색 판정을 `CanPlaceMachine` 하나로 모았습니다. 호버 색과 클릭 판정이 같은 함수를 거치는 한 "보이는 대로 놓인다"가 구조적으로 보장돼요. 같은 invariant를 두 곳에서 따로 구현하지 않는 것 — 2편의 단일 진입점과 같은 원칙입니다.
